...
void renderText(SDL_Renderer *m_renderer, int xp, int yp, int w, int h, string addressOfFont, int fontsize, int R, int G, int B, int A, string S);
...
void game();
void updateScore();
void sortScore(string names[], int tedad, int lists[][8]);
SDL_Rect ball, backg;
SDL_Texture *backimg[2], *ballimg[4], *stars[10], *portal[18];
// Index của quả bóng và sân đc sử dụng
int ballnum, backnum;
int backi, ballstate, timep;
bool isRunning;
double vx, vy;
Uint32 start;
class Player
...
public:
    ...
    double pvy, pvx;
    int goalnum;
    int playerIndex; // Index của cầu thủ được chọn
    SDL_Point shoescenter;
    string name;
    double powline; // Thanh năng lượng để sử dụng skill
    int timepower;
    bool shoot, pow[7], booltimer, sshoot, freez;
};

Player player1, player2;
// Ảnh của đôi giày và nắm đấm cầu thủ
SDL_Texture *shoesImg, *punchimg;

struct menu
{
    // Menu có đang ở trạng thái đầu tiên hay k
    bool initStatus;
    // Bật, tắt nhạc nền, hiệu ứng âm thanh
    bool music, sound;
    // Background của menu
    ...
    // Khối option các chế độ của game
    SDL_Texture *clawmenu;
    SDL_Rect clawMenuRect;
    // Menu đầu tiên của game
    void initMenu(bool &music);
    // Chọn cầu thủ thi đấu
    void selectPlayer();
    // Chọn bóng và chọn sân
    void otherSelection();
    // Nhập tên player
    void selectName();
    string inputName(string pname, int x, int y, int &k);
    // BXH ở menu đầu tiên
    void scoreboard();
    // BXH khi trận đấu chuẩn bị bắt đầu
    void scoreboardPlay();
};
menu Menu;
struct Sounds
{
    ...
    // Các âm thanh hiệu ứng nhỏ khác
    void dingSound()
    {
        if (Menu.sound)
        {
...
void menu::initMenu(bool &music)
{
    ...
    // Load các image
    menuBackgroundRect = {0, 0, SCREEN_WIDTH, SCREEN_HEIGHT};
    ...
    menuBackground = SDL_CreateTextureFromSurface(renderer, tempts);
    ...
    while (initStatus)
    {
        ...
        if (event.type == SDL_QUIT)
            SDL_Quit();
        if (event.type == SDL_KEYDOWN) // Lựa chọn các chế độ
            ...
            case (SDLK_LEFT):
                ...
                break;
            case (SDLK_SPACE):
                if (menuIndex == 0 || (menuIndex == 1 && timep != 0) || menuIndex == 2)
                    initStatus = false;
                if (menuIndex == 4)
                {
                    music = !music;
        ...
        thickLineRGBA(renderer, clawMenuRect.x + 200, clawMenuRect.y + 10, clawMenuRect.x + 200, 0, 40, 0, 0, 0, 255);
        SDL_RenderCopy(renderer, clawmenu, NULL, &clawMenuRect);
        boxRGBA(renderer, 50, 700, SCREEN_WIDTH - 30, SCREEN_HEIGHT - 20, 255, 255, 255, 190);

        if (menuIndex >= 3) // Chế độ setting
        {
            SDL_RenderCopy(renderer, setting, NULL, &settingRect);
            if (menuIndex == 4)
                filledEllipseRGBA(renderer, 1108, 745, 35, 35, (!music) ? 255 : 0, (music) ? 255 : 0, 0, 100);
            if (menuIndex == 5)
                filledEllipseRGBA(renderer, 1200, 745, 35, 35, (!sound) ? 255 : 0, (sound) ? 255 : 0, 0, 100);
        }
        // Render text của các option
        renderText(renderer, 100, 690, 210, 100, "fonts/FreeSans.ttf", 90, (menuIndex == 0) ? 255 : 0, 0, 0, 255, "New game");
        renderText(renderer, 350, 690, 300, 100, "fonts/FreeSans.ttf", 90, 0, (menuIndex == 1) ? 255 : 0, 0, 255, "resume game");
        renderText(renderer, 700, 690, 120, 100, "fonts/FreeSans.ttf", 90, 0, 0, (menuIndex == 2) ? 255 : 0, 255, "table");
        renderText(renderer, 850, 690, 200, 100, "fonts/FreeSans.ttf", 90, (menuIndex == 3) ? 255 : 0, (menuIndex == 3) ? 255 : 0, 0, 255, "settings");
...
void menu::scoreboard()
{
    ...
    while (st)
    {
        ...
        for (int v = 0; v < q; v++)
        {
            renderText(renderer, 450, 215 + 35 * v, names[v].length() * 13, 30, "fonts/FreeSans.ttf", 30, 255, 255, 255, 255, names[v]);
            for (int u = 0; u < 8; u++)
            {
                if (lists[v][u] < 10)
                    renderText(renderer, 750 + 55 * u, 215 + 35 * v, 18, 30, "fonts/FreeSans.ttf", 30, 255, 255, 255, 255, to_string(lists[v][u]));
                else
                    renderText(renderer, 750 + 55 * u, 215 + 35 * v, 36, 30, "fonts/FreeSans.ttf", 30, 255, 255, 255, 255, to_string(lists[v][u]));
            }
        }
    ...
    fin.close();
    sounds.ding2Sound();
    Menu.initMenu(Menu.music);
}
void menu::scoreboardPlay()
{
    SDL_Surface *tempts = IMG_Load("images/score table.png");
    menuBackground = SDL_CreateTextureFromSurface(renderer, tempts);
    SDL_FreeSurface(tempts);
    renderText(renderer, 380, 10, 40, 30, "fonts/FreeSans.ttf", 40, 255, 255, 200, 255, "Lets GO!");
    ...
    while (st)
    {
        ...
        SDL_RenderCopy(renderer, menuBackground, NULL, &menuBackgroundRect);
        boxRGBA(renderer, 10, 695, 100, 790, 0, 0, 0, 175);
        renderText(renderer, 450, 30, 340, 100, "fonts/FreeSans.ttf", 100, 0, 250, 80, 255, "Lets GO!");
        ...
        for (int v = 0; v < q; v++)
        {
            // Nếu tên player trùng tên trên BXH sẽ in màu đỏ
            if (name[v] == player1.name)
            {
                renderText(renderer, 450, 215 + 35 * v, name[v].length() * 13, 30, "fonts/FreeSans.ttf", 30, 255, 25, 25, 255, name[v]);
                t1 = v;
            }
            else if (name[v] == player2.name)
            {
                ...
                renderText(renderer, 450, 215 + 35 * v, name[v].length() * 13, 30, "fonts/FreeSans.ttf", 30, 255, 255, 255, 255, name[v]);
            for (int u = 0; u < 8; u++)
            {
                if (v == t1 || v == t2)
                {
                    if (lists[v][u] < 10)
                        renderText(renderer, 750 + 55 * u, 215 + 35 * v, 18, 30, "fonts/FreeSans.ttf", 30, 255, 25, 25, 255, to_string(lists[v][u]));
                    else
                        renderText(renderer, 750 + 55 * u, 215 + 35 * v, 36, 30, "fonts/FreeSans.ttf", 30, 255, 25, 25, 255, to_string(lists[v][u]));
                }
                else
                {
                    if (lists[v][u] < 10)
...
void menu::selectPlayer()
{
    ...
    SDL_Texture *playerImg[8];
    // Background mới
    SDL_Surface *tempts = IMG_Load("images/mback.png");
    menuBackground = SDL_CreateTextureFromSurface(renderer, tempts);

    // Load hình ảnh các cầu thủ
    for (int i = 0; i < 8; i++)
    {
        tempts = IMG_Load(("images/players/p" + to_string(i) + ".png").c_str());
        playerImg[i] = SDL_CreateTextureFromSurface(renderer, tempts);
    }
    ...
    while (selectState)
    {
        ...
        if (e.type == SDL_QUIT)
            SDL_Quit();
        if (e.type == SDL_KEYDOWN) // Lựa chọn cầu thủ
            ...
            case (SDLK_DOWN):
                ...
                {
                    player2.playerIndex++;
                    if (player2.playerIndex <= 7)
                        sounds.ding3sound();
                    if (player2.playerIndex > 7)
                        player2.playerIndex = 7;
                }
                break;
            case (SDLK_UP):
                if (k == 1)
        ...
        player1.playerImg = playerImg[player1.playerIndex];
        player2.playerImg = playerImg[player2.playerIndex];
        ...
        SDL_RenderCopy(renderer, player1.playerImg, NULL, &player1.pos);
        SDL_RenderCopyEx(renderer, player2.playerImg, NULL, &player2.pos, 0, 0, SDL_FLIP_HORIZONTAL);
        //        SDL_SetRenderDrawColor(renderer,0,0,0,255);
        ...
        // Mô tả các chiêu thức đặc biệt của player
        ...
        mymap[6] = "   No Power   ";
        mymap[7] = "     Thief    ";
        renderText(renderer, 370, 230, 200, 85, "fonts/FreeSans.ttf", 50, 255, 0, 0, 255, mymap[player1.playerIndex]);
        renderText(renderer, 690, 230, 200, 85, "fonts/FreeSans.ttf", 50, 255, 0, 0, 255, mymap[player2.playerIndex]);
        SDL_RenderPresent(renderer);
        SDL_Delay(5);
        SDL_SetRenderDrawColor(renderer, 0, 0, 0, 255);
        SDL_RenderClear(renderer);
...
void menu::otherSelection()
{
    ...
    player2.pos = {50, 50, SCREEN_WIDTH - 100, SCREEN_HEIGHT - 100};
    ...
    while (select)
    {
        ...
        renderText(renderer, 720, 460, 200, 85, "fonts/FreeSans.ttf", 50, 255, 255, 255, 255, "soccer field");
        SDL_RenderPresent(renderer);
        SDL_Delay(5);
        SDL_SetRenderDrawColor(renderer, 0, 0, 0, 255);
        SDL_RenderClear(renderer);
...
void menu::selectName()
{
    ...
    while (nameStatus)
    {
        ...
        renderText(renderer, 30, 480, 260, 90, "fonts/FreeSans.ttf", 50, 255, 255, 255, 255, "Player1 name:");
        renderText(renderer, 900, 480, 260, 90, "fonts/FreeSans.ttf", 50, 255, 255, 255, 255, "Player2 name:");
        renderText(renderer, 30, 580, player1.name.length() * 25, 80, "fonts/FreeSans.ttf", 50, 255, 255, 255, 255, player1.name);
        renderText(renderer, 900, 580, player2.name.length() * 25, 80, "fonts/FreeSans.ttf", 50, 255, 255, 255, 255, player2.name);
        switch (k)
        {
        case (0):
            boxRGBA(renderer, 10, 695, 100, 790, 0, 0, 0, 175);
            break;
        case (1):
            player1.name = inputName(player1.name, 30, 530, k);
            break;
        case (2):
            player2.name = inputName(player2.name, 900, 530, k);
            break;
        case (3):
            boxRGBA(renderer, 1175, 695, 1265, 790, 0, 0, 0, 175);
            break;
...
string menu::inputName(string pname, int x, int y, int &k)
{
    TTF_Init();
    SDL_StartTextInput();
    SDL_Event e;
    ...
    while (nameStatus)
    {
        ...
        boxRGBA(renderer, 30, 580, 380, 680, 0, 0, 0, 200);
        boxRGBA(renderer, 900, 580, 1250, 680, 0, 0, 0, 200);
        renderText(renderer, x, y + 50, pname.length() * 25, 80, "fonts/FreeSans.ttf", 50, 255, 255, 255, 255, pname);
        a++;
        if (a > 19)
            a = 0;
        renderText(renderer, x + pname.length() * 25, y + 45, 30, 90, "fonts/FreeSans.ttf", 50, 255, 255, 255, (a / 10) ? 1 : 255, "|");
        renderText(renderer, 30, 480, 260, 90, "fonts/FreeSans.ttf", 50, 255, 255, 255, 255, "Player1 name:");
        renderText(renderer, 900, 480, 260, 90, "fonts/FreeSans.ttf", 50, 255, 255, 255, 255, "Player2 name:");
        if (k == 2)
            renderText(renderer, 30, 580, player1.name.length() * 25, 80, "fonts/FreeSans.ttf", 50, 255, 255, 255, 255, player1.name);
...
void endMatch()
{
    ...
    if (player1.goalnum > player2.goalnum)
        s = player1.name + " won! ";
    else if (player2.goalnum > player1.goalnum)
        s = player2.name + " won! ";
    else
        s = "Equal! ";
    // Update điểm số vào file thống kê
    updateScore();
    int num = 0;
    bool endMatch = true;
    SDL_Event e;
    // Các tùy chọn ở màn hình kết thúc
    while (endMatch)
    {
        SDL_PollEvent(&e);
        if (e.type == SDL_KEYDOWN)
        ...
        boxRGBA(renderer, 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, 0, 0, 0, 155);
        SDL_RenderCopy(renderer, resultImg, NULL, &resultrect);
        SDL_RenderCopy(renderer, player1.playerImg, NULL, &player1.pos);
        SDL_RenderCopyEx(renderer, player2.playerImg, NULL, &player2.pos, 0, 0, SDL_FLIP_HORIZONTAL);
        SDL_RenderCopy(renderer, shoesImg, NULL, &player1.shoes);
        SDL_RenderCopyEx(renderer, shoesImg, NULL, &player2.shoes, 0, 0, SDL_FLIP_HORIZONTAL);
        renderText(renderer, 540, 390, 200, 70, "fonts/FreeSans.ttf", 70, 255, 255, 255, 255, to_string(player1.goalnum) + " - " + to_string(player2.goalnum));
        renderText(renderer, 540, 450, 200, 60, "fonts/FreeSans.ttf", 70, 255, 255, 255, 255, s.c_str());
        switch (num)
        {
        case (0):
...
void initialize()
{
    ...
    SDL_FreeSurface(tempts);
    backg = {0, 40, SCREEN_WIDTH, SCREEN_HEIGHT - 80};
    //////////////////////////////////////////////////////////////////
    ...
    while (player1.pos.y < 430 - 2)
    {
        ...
        SDL_RenderCopyEx(renderer, player2.playerImg, NULL, &player2.pos, NULL, NULL, SDL_FLIP_HORIZONTAL);
        boxRGBA(renderer, 0, 0, SCREEN_WIDTH, 40, 0, 0, 0, 255);
    ...
    /////////////////////////////////
    backi = 0;
    ball = {SCREEN_WIDTH / 2 - 30, 450 - 30, 60, 60};
    ballstate = 0;
    vx = 0;
    vy = 0;
    player1.goalnum = 0;
...
void handleEvent()
{
    ...
    if (!player1.freez)
    {
        if (kstate[SDL_SCANCODE_D] && player1.pos.x < SCREEN_WIDTH - 115 && !(player2.pos.x - player1.pos.x < 100 && player2.pos.x > player1.pos.x && player1.pos.y > 245))
            player1.pvx = 30;
        ...
        if (kstate[SDL_SCANCODE_C])
        {
            if (player1.powline == LP)
            {
                player1.pow[player1.playerIndex] = true;
                if (player1.playerIndex == 5 || player1.playerIndex == 7)
                    player1.pow[rand() % 5] = true;
    ...
    if (!player2.freez)
    {
        if (kstate[SDL_SCANCODE_L] && player2.pos.x < SCREEN_WIDTH - 115 && !(player1.pos.x - player2.pos.x < 100 && player1.pos.x > player2.pos.x && player2.pos.y > 245))
            player2.pvx = 30;
        ...
        if (kstate[SDL_SCANCODE_N])
        {
            if (player2.powline == LP)
            {
                player2.pow[player2.playerIndex] = true;
                if (player1.playerIndex == 5 || player1.playerIndex == 7)
                    player2.pow[rand() % 5] = true;
...
void ballplaying()
{
    ...
    if (y - vy * t < 625 && !(player1.sshoot || player2.sshoot))
        vy += g * t;
    if (y >= 625)
    {
...
void render()
{
    ...
    if (player1.pow[1])
    {
        ...
        SDL_RenderCopy(renderer, player1.playerImg, NULL, &pc1);
        SDL_RenderCopyEx(renderer, shoesImg, NULL, &psc2, player1.theta, &player1.shoescenter, SDL_FLIP_NONE);
        SDL_RenderCopy(renderer, player1.playerImg, NULL, &pc2);
    }
    if (player2.pow[1])
    {
        ...
        SDL_RenderCopyEx(renderer, player2.playerImg, NULL, &pc1, NULL, NULL, SDL_FLIP_HORIZONTAL);
        SDL_RenderCopyEx(renderer, shoesImg, NULL, &psc2, player2.theta, &player2.shoescenter, SDL_FLIP_HORIZONTAL);
        SDL_RenderCopyEx(renderer, player2.playerImg, NULL, &pc2, NULL, NULL, SDL_FLIP_HORIZONTAL);
    }
    static int a = 0, b = 0;
    SDL_Rect starrect[2];
    if (player1.freez)
    ...
    if (player1.playerIndex != 0 && player1.playerIndex != 6)
        thickLineRGBA(renderer, 482, 175, 482, 175 - player1.powline, 15, 255, 0, 0, 255);
    if (player2.playerIndex != 0 && player2.playerIndex != 6)
    ...
    if (player1.pow[player1.playerIndex])
    {
        if (player1.booltimer)
        {
            player1.timepower = (SDL_GetTicks() - start) / 1000;
    ...
    if (player2.pow[player2.playerIndex])
    {
        if (player2.booltimer)
        {
            player2.timepower = (SDL_GetTicks() - start) / 1000;
    ...
    for (int i = 0; i < 5; i++)
    {
        if (player1.pow[i] && player1.playerIndex)
            renderText(renderer, 200, 750, 200, 50, "fonts/FreeSans.ttf", 50, 255, 0, 0, 255, mymap[i]);
        if (player2.pow[i] && player2.playerIndex)
            renderText(renderer, 900, 750, 200, 50, "fonts/FreeSans.ttf", 50, 255, 0, 0, 255, mymap[i]);
    }
    renderText(renderer, 690, 95, 70, 100, "fonts/FreeSans.ttf", 70, 255, 255, 255, 255, to_string(player2.goalnum));
    renderText(renderer, 520, 95, 70, 100, "fonts/FreeSans.ttf", 70, 255, 255, 255, 255, to_string(player1.goalnum));
    renderText(renderer, 620, 110, 40, 65, "fonts/FreeSans.ttf", 70, 255, 255, 255, 255, to_string(timep));
    renderText(renderer, 510, 45, 110, 35, "fonts/FreeSans.ttf", 60, 255, 255, 255, 255, player1.name.c_str());
    renderText(renderer, 660, 45, 110, 35, "fonts/FreeSans.ttf", 60, 255, 255, 255, 255, player2.name.c_str());
...
void updateScore()
{
    ...
    {
        if (player1.name == names[i])
            t1 = i;
        if (player2.name == names[i])
            t2 = i;
    }
...
void goalcelebrate()
{
    ...
    for (int i = 0; i < 25; i++)
    {
        ...
        render();
        boxRGBA(renderer, 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, 0, 0, 0, 150);
        texture(renderer, 200, 20, goal + ".gif", SCREEN_WIDTH - 400, SCREEN_HEIGHT - 100);
        SDL_RenderPresent(renderer);
        SDL_SetRenderDrawColor(renderer, 0, 0, 0, 255);
        SDL_RenderClear(renderer);
        SDL_Delay(40);
